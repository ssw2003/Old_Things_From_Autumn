...
{
        char* wds[MAXARGS];
        int commandPlaceThing[MAXARGS];
        int standardInGo[MAXARGS];
        int standardOutGo[MAXARGS];
        int i = parseline(cmdline, wds);
        if (i > 0) { i = 0; }
        if (i < 0) { i = 0; }
        i = 1 - i;
        if (builtin_cmd(wds) && i) { exit(0); }
        i = parseargs(wds, commandPlaceThing, standardInGo, standardOutGo);
        if (i == 0) { return; }
        int u = 0;
        int pf[2];
        int pg[2];
        ...
        while (u < i) {
                ...
                if (u == 0 && (fk != 0)) {
                        v = fk;
                        if (getpgid(getpid()) == v) { v = v + 1; }
                }
                if (fk == 0) {
                        int inDs = 0;
                        if (standardInGo[u] >= 0) { inDs = open(wds[standardInGo[u]], O_RDONLY, 0600); }
                        if (inDs > 0) { dup2(inDs, 0); }
                        if (inDs > 0) { close(inDs); }
                        int outDs = 1;
                        if (standardOutGo[u] >= 0) { outDs = open(wds[standardOutGo[u]], O_WRONLY | O_CREAT | O_TRUNC, 0600); }
                        if (outDs != 1) { dup2(outDs, 1); }
                        if (outDs != 1) { close(outDs); }
                        if (u != i - 1) {
                                close(pf[0]);
                                dup2(pf[1], 1);
                                close(pf[1]);
                        ...
                        if (execve(wds[commandPlaceThing[u]], commandPlaceThing[u] + wds, environ) < 0) { i = i; }
                        u = i - 1;
                        exit(0);
                } else {
                        setpgid(fk, v);
